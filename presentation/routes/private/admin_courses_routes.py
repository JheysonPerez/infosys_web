...
def create_course():
    if request.method == "POST":
        ...
        if cert_name:
            data["certificate_image"] = f"images/certificates/{cert_name}"
...
def edit_course(course_id):
    ...
    if request.method == "POST":
        ...
        if image_file and image_file.filename:
            if course.image:
                old_path = os.path.join(current_app.root_path, "static", "images", "courses", course.image)
        ...
        if cert_file and cert_file.filename:
            if course.certificate_image:
                old_cert_path = os.path.join(current_app.root_path, course.certificate_image)
...
def delete_course(course_id):
    ...
    if course:
        if course.image:
            image_path = os.path.join(current_app.root_path, "static", "images", "courses", course.image)
            if os.path.exists(image_path):
                os.remove(image_path)
        if course.certificate_image:
            cert_path = os.path.join(current_app.root_path, course.certificate_image)
